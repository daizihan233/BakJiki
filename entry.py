...
old_d = {}
try:
    os.makedirs('./jikipedia_backup_entry/')
except FileExistsError:
    pass
while True:
    data = jiki.request_created_definition(page=page, category="normal")
    if old_d == data:
        break
    else:
        try:
    ...
    try:
        for d in data['data']:
            work_pwd = f'./jikipedia_backup_entry/{d["id"]}-{d["term"]["title"]}'
            try:
                os.makedirs(work_pwd)
            except FileExistsError:
                continue
            except OSError:
                work_pwd = f'./jikipedia_backup_entry/{d["id"]}'
                try:
                    os.makedirs(work_pwd)
                except FileExistsError:
                    continue
